...
import { ADD_EXPENSE } from '../../../graphql/expense/mutation';
...
const ExpenseForm: React.FC<ExpenseFormProps> = ({ operationSuccess }) => {
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values) => {
            try {
                let createdExpense = await addExpense({ variables: { ...values, amount: parseFloat(values.amount) } });
                const { success, message } = graphqlResponseSerializer(createdExpense, 'addExpense');
                if (success) {
                    operationSuccess()
            ...
            <div className='rounded-lg border bg-card text-card-foreground shadow-sm mx-auto max-w-md'>
                <div className='p-6'>
                    <form className='space-y-4' onSubmit={formik.handleSubmit}>
                        ...
                        <Button type="submit">{t("expense.add_expense")}</Button>
                    </form>
                </div>
            </div>
